                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <div v-if="uri && twoFAStatus == false" class="mt-3">
                                ...
                                <p v-show="tokenValid" class="mt-2" style="color: var(--arbour-primary);">{{ $t("tokenValidSettingsMsg") }}</p>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        /** Show the dialog */
        submit() {
            // form submit handler — no default action
        },
        ...
        confirmEnableTwoFA() {
            (this.$refs.confirmEnableTwoFA as { show(): void }).show();
        },

        /** Show dialog to confirm disabling 2FA */
        confirmDisableTwoFA() {
            (this.$refs.confirmDisableTwoFA as { show(): void }).show();
        },
        ...
        prepare2FA() {
            ...
            this.$root.getSocket().emit("prepare2FA", this.currentPassword, (res: SocketRes) => {
                this.processing = false;

                if (res.ok) {
                    this.uri = res.uri as string | null;
                } else {
                    toast.error(res.msg);
                }
            });
        ...
        save2FA() {
            ...
            this.$root.getSocket().emit("save2FA", this.currentPassword, (res: SocketRes) => {
                this.processing = false;
        ...
        disable2FA() {
            ...
            this.$root.getSocket().emit("disable2FA", this.currentPassword, (res: SocketRes) => {
                this.processing = false;
        ...
        verifyToken() {
            this.$root.getSocket().emit("verifyToken", this.token, this.currentPassword, (res: SocketRes) => {
                if (res.ok) {
                    this.tokenValid = res.valid as boolean;
                } else {
                    toast.error(res.msg);
                }
            });
        ...
        getStatus() {
            this.$root.getSocket().emit("twoFAStatus", (res: SocketRes) => {
                if (res.ok) {
                    this.twoFAStatus = res.status as boolean | null;
                } else {
                    toast.error(res.msg);
                }
            });
