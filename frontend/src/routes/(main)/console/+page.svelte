...
onMount(() => {
    socketStore.emitAgent("", "checkMainTerminal", (res: { ok: boolean }) => {
...
{#if !loading}
    ...
    {:else}
        <div class="alert">
            ...
            <!-- eslint-disable-next-line svelte/no-at-html-tags -->
            <p>{@html $t("ConsoleNotEnabledMSG2")}</p>
            <!-- eslint-disable-next-line svelte/no-at-html-tags -->
            <p>{@html $t("ConsoleNotEnabledMSG3")}</p>
        </div>
    {/if}
{/if}
...
.alert {
    background: color-mix(in srgb, var(--arbour-warning) 12%, transparent);
    border: 1px solid var(--arbour-warning);
    border-radius: var(--arbour-radius-lg);
    padding: 1rem 1.25rem;
}
.alert h4 { font-size: 1rem; color: var(--arbour-text-on-warning); margin: 0 0 0.5rem; }
