...
export class Stack {
    ...
    constructor(server: ArbourServer, name: string, composeYAML?: string, composeENV?: string) {
        this.name = name;
        this.server = server;
    ...
    getSimpleData(endpoint : string) : SimpleStackData {
        return {
            ...
            tags: [],
            isManagedByArbour: this.isManagedByArbour,
            composeFileName: this._composeFileName,
            endpoint
        };
    }

    get isManagedByArbour() : boolean {
    ...
    async deploy(socket : ArbourSocket) : Promise<number> {
    ...
    async delete(socket: ArbourSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    static async getStackList(server : ArbourServer, useCacheForManaged = false) : Promise<Map<string, Stack>> {
        ...
        for (let composeStack of composeList) {
            ...
            // This stack probably is not managed by Arbour, but we still want to show it
            if (!stack) {
                // Skip the arbour stack if it is not managed by Arbour
                if (composeStack.Name === "arbour") {
                    continue;
                }
    ...
    static async getStack(server: ArbourServer, stackName: string, useCache = true) : Promise<Stack> {
    ...
    async start(socket: ArbourSocket) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
        let exitCode = await Terminal.exec(this.server, socket, terminalName, "docker", [ "compose", "up", "-d", "--remove-orphans" ], this.path);
        if (exitCode !== 0) {
    ...
    async stop(socket: ArbourSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async restart(socket: ArbourSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async stopService(socket: ArbourSocket, service: string): Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async startService(socket: ArbourSocket, service: string): Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async restartService(socket: ArbourSocket, service: string): Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async recreateService(socket: ArbourSocket, service: string): Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async down(socket: ArbourSocket) : Promise<number> {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async update(socket: ArbourSocket, pruneAfterUpdate: boolean, pruneAllAfterUpdate: boolean) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async updateService(socket: ArbourSocket, service: string, pruneAfterUpdate: boolean, pruneAllAfterUpdate: boolean) {
        const terminalName = getComposeTerminalName(socket.endpoint, this.name);
    ...
    async joinContainerTerminal(socket: ArbourSocket, serviceName: string, shell : string = "sh", index: number = 0) {
    ...
    async joinContainerLog(socket: ArbourSocket, serviceName: string, index: number = 0) {
